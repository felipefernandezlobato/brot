                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <Link href={p.receta_id && p.nivel === "terminado" ? `/escandallos/${p.receta_id}` : `/congelados/${p.id}`} className="font-medium text-brot text-sm truncate hover:underline" onClick={(e) => e.stopPropagation()}>{p.nombre}</Link>
                  ...
                  {allProductsWithData.map((prod, idx) => (
                    ...
                    >
                      <td className="px-3 py-1.5 font-medium text-text sticky left-0 bg-white z-10 whitespace-nowrap" style={idx % 2 !== 0 ? { backgroundColor: "rgb(245 240 232 / 0.3)" } : undefined}>
                        <Link href={prod.receta_id && prod.nivel === "terminado" ? `/escandallos/${prod.receta_id}` : `/congelados/${prod.id}`} className="hover:text-brot hover:underline">{prod.nombre}</Link>
                      </td>
