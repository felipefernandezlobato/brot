...
### Pizza / Pan Lomo (extra-only, no fixed day; Pan Lomo added 2026-09-03)
Same two-stage masa→terminado chain as Focaccia, but produced as-needed rather than on a fixed weekly day, so there is deliberately no `TareaProduccion` row for either — production is logged directly as an "extra" `RegistroProduccion` (`tarea_id=None`, `titulo_extra` set to the product name, `receta_id`/`producto_congelado_id` set to the masa or terminado stage being logged).
- Pizza: Receta 7 "Pizza 350g" (masa, `porciones_por_lote=1`) → Receta 44 "Formar y Cocinar Pizza" (terminado, `porciones_por_lote=20`, references Receta 7 as a `subreceta_id` line). ProductoCongelado 46 "Masa de Pizza" (`nivel="masa"`, `receta_id=7`) → ProductoCongelado 20 "Pizza Cocinado" (`nivel="terminado"`, `receta_id=44`, `producto_padre_id=46`, `cantidad_por_padre=20`). Batch: 300g Semolín, 3700g Harina 00 Pizza, 20g Levadura, 110g Sal, 2.6L Agua, 0.15L Aceite Oliva, 0.05L Tomate.
- Pan Lomo: identical batch minus the Tomate line. Receta 45 "Masa de Pan Lomo" (masa) → Receta 46 "Formar y Cocinar Pan Lomo" (terminado, `porciones_por_lote=27`, `precio_venta=875`). ProductoCongelado 47 "Masa de Pan Lomo" (masa) → ProductoCongelado 48 "Pan Lomo Cocinado" (terminado, `producto_padre_id=47`, `cantidad_por_padre=27` — yield differs from Pizza's 20).
- Pan Lomo was set up with no production history logged yet (Felipe: "fue antes de que tuviéramos el sistema") — only a 1u `dañado` merma (2026-09-03) and two manual counts (9u @ 2026-08-13, 8u @ 2026-09-03) exist. Calculado will show as a discrepancy against both counts until real production gets logged; this is expected, not a bug.
- **Gotcha: `ProductoCongeladoCreate`/`ProductoCongeladoUpdate` (`schemas.py`) do not expose `receta_id`/`nivel`/`producto_padre_id`/`cantidad_por_padre` at all** — `POST`/`PUT /api/congelados/productos` cannot set up a new masa→terminado chain. There is no dedicated endpoint for it either. Setting these fields means constructing `ProductoCongelado` rows directly via the SQLAlchemy model (a plain `db.add()`/`commit()`, mirroring the shape of existing rows) rather than through the API. This is catalog/config data, not the append-only ledger — it does NOT fall under the "never hand-edit `MovimientoStock`/`StockCongelado`" rule above, which is specifically about stock *movements*, not product/recipe catalog rows.
- **Same gotcha exists one layer up, on `ProductoCatalogo.receta_id`** (found 2026-09-10 on Pan Lomo): `ProductoCatalogoCreate`/`Update` don't expose `receta_id` either, so a catalog item created for B2B/customer-portal sale can end up with `receta_id=NULL` even though its `ProductoCongelado`/`Receta` chain is fully set up. `deducir_congelado_por_catalogo()` (`services/stock.py`) returns `None` immediately when `cat.receta_id` is falsy, which surfaces as a 422 ("No se pudo resolver el stock congelado para 'X'") the moment anyone tries to mark a B2B entrega containing that product as `entregado` — blocking the WHOLE entrega, not just that product's line. Fix is the same as above: a plain `UPDATE productos_catalogo SET receta_id=...` directly on the row, not through any endpoint.
...
- **Product names should match across every stage of a chain** (renamed 2026-09-10): the `crudo` stage of the Pan de Choco chain was named "Napolitana Cruda" while its `terminado` was "Pan de Choco Cocinado" — same product, inconsistent name at each step made the chain (`/escandallos/[id]`'s "Cadena de produccion completa") harder to read. Renamed the crudo `ProductoCongelado` to "Pan de Choco Crudo" (plain `nombre` update, no chain/receta_id change). The underlying `Receta`/`ProductoCongelado` ids and the recipe's own name ("Napolitana", the terminado's recipe) were deliberately left alone — only the crudo *product* name changed.
...
**`ajustar_correccion_conteo()`/`_revertir_correccion_conteo()` in `services/stock.py`** is the sanctioned primitive for the rarer case that DOES need the ledger to move — a genuinely wrong `carga_inicial` baseline (a `carga_inicial` was never a real event, just a best guess at setup time, so editing it isn't erasing evidence of anything). Wired into `POST /api/congelados/{id}/sincronizar-ledger` and its materia_prima twin in `inventario.py` (both one-off, permission-gated, re-anchor `carga_inicial` to "this row's current corrected value"). Exception to "never mutate a MovimientoStock row": if the only prior ledger movement is a lone `carga_inicial` dated the SAME day as the row being corrected, it's edited in place instead of leaving a correction parked next to it — but only same-day; correcting a LATER date's count must never overwrite an EARLIER carga_inicial (a real bug found and fixed the same day). Covered by direct unit tests (`tests/test_ajustar_correccion_conteo.py`), not through the routers.

**Gotcha (found and fixed 2026-08-21, same day the congelado endpoint shipped):** `sincronizar-ledger` for congelado passed the lot's `cantidad` as "the corrected value" — wrong once any of that lot has been drawn down by FIFO, since `cantidad` then means "what's left", not "what was counted". Calling it against an already-consumed lot silently rewrote a correct `carga_inicial` down to the post-consumption remainder. Must pass `cantidad_original` instead (see below) — materia_prima's `InventarioRegistro.cantidad` has no such split, so its twin never had this bug.

### Calculado re-anchors to the physical count (added 2026-08-21, refined same day)
...
**`cantidad_original` on `StockCongelado` (added 2026-08-21, migration `4dac5d6d7328` + `6084d95ab102`).** `_conteos_manuales_por_fecha()` used to read a congelado lot's `cantidad` directly for the anchor — wrong the moment any of that lot gets drawn down by FIFO, since `cantidad` mutates in place (`deducir_congelado_fifo`) while the anchor needs the fixed original count. A same-day live fix that tried reconstructing it as `cantidad + still-live ConsumoFifoDetalle draws` broke calculado for most of the catalog within the hour and was reverted: `historial_movimientos_acumulado`'s own per-movement loop *also* re-applies every one of those same draws via the ledger, so any lot with more than one subsequent draw got its consumption subtracted twice. `cantidad_original` is the real fix — set once at lot creation (`add_stock_congelado`, `crear_lote_ajuste`, `producir_producto`'s output lot, `_restaurar_lotes`' reconstructed lot, `load_historical.py`) and never touched again by consumption, a true immutable snapshot instead of a value re-derived from time-varying joins. The migration backfilled existing rows once with that same `cantidad + draws` formula, but ONLY for lots sitting at exactly 0 (`deducir_congelado_fifo`'s unambiguous full-depletion signature) — a lot with a subtler pre-existing inconsistency (found on Croissant Cocinado's 2026-08-13 lot, still at its full un-reduced value despite live draws recorded against it — an unrelated older edit/reversal edge case) got the formula applied wrong too, so the second migration narrowed it back to "only touch lots at exactly 0, leave everything else as `cantidad_original = cantidad`". **`PUT /api/congelados/{id}` must set `cantidad_original = cantidad` whenever `cantidad` is in the edit** — otherwise correcting a mistyped count doesn't propagate to calculado for any date after it, silently reintroducing the exact bug this field exists to fix.

**Reflecting a count with nothing moved since (added 2026-08-21, generalized 2026-09-10).** The reset above only fired while iterating a movement dated AFTER the count — if nothing (no production/sale/merma) happened since, that line of the loop never runs, so calculado for "today" kept showing the stale pre-count trajectory forever, effectively ignoring the count. This was the common case, not the exception (confirmed live against 8 materia_prima ingredients). The original fix only emitted a trailing point for an anchor with NO movement at all before `fecha_hasta`/today — it missed the equally common case of an anchor with a LATER movement that just isn't the very next day (Felipe: counted Croissant Cocinado at 173 on 9/3, next movement wasn't until 9/5, so 9/4 kept showing the stale pre-count 199 — the reset was correctly applied on 9/5, just never surfaced for the gap day). Fixed by folding "the day after every anchor" into the same set of checkpoint dates as real movements (not a separate trailing-only special case), so a reset checkpoint always exists immediately after a count regardless of when the next real movement happens to land. Relies on the frontend's own pivot lookup (`valorCalculadoEnFecha`, duplicated identically in `congelados/page.tsx` and `stock/page.tsx`: latest point with `fecha <= column`) to carry that value forward through every later date column — no frontend change needed.

**`deducir_congelado_fifo` is now date-bound, and `cantidad_original` finally reaches the UI (fixed 2026-09-03).** `cantidad_original` (above) protected the backend's OWN reset math since 2026-08-21, but `StockCongeladoOut` never actually exposed the field — the frontend historial pivot's "contado" (parenthesized) figure and its chart (`congelados/page.tsx`, `pivotData`/`rawByDate`) were still built from `.cantidad`, the live FIFO-mutated remaining balance. Separately, `deducir_congelado_fifo` had no date bound at all — it drew from whichever `is_active=True` lot was oldest, with no check that the lot's `fecha_entrada` was on-or-before the movement's own `fecha`. Combined, these let a backdated delivery (entered days after the fact) reach into a lot dated LATER than itself — e.g. eating a fresh physical count from today — whenever every correctly-dated lot for that product had already cycled through to `is_active=False`. Surfaced as a manual count's displayed number visibly changing on its own with no one touching it, purely from reprocessing unrelated backdated entregas.
- Fix: `deducir_congelado_fifo` filters `StockCongelado.fecha_entrada <= (fecha or date.today())` before drawing — a genuine historical shortfall still surfaces correctly as a visible negative balance via the existing `crear_lote_ajuste` path, it just can no longer borrow from the future to hide it. `cantidad_original` added to `StockCongeladoOut`/`_stock_out`, and every frontend read of "what was physically counted" (`pivotData`, `rawByDate`, `iniciarEdicion`'s starting value) switched from `.cantidad` to `.cantidad_original`. Lot-list views that legitimately show live remaining stock (Registrar/current-stock tabs) were left reading `.cantidad` — different, correct use of the mutable field.
- **Invariant going forward: "lo contado" must never move except by an explicit edit to that specific count** — not from reversals, backdated entregas, FIFO consumption, or any other unrelated action. Any new code that could mutate `StockCongelado.cantidad` must leave `cantidad_original` untouched; any new UI surface showing "the count" must read `cantidad_original`, never `cantidad`.
- **Why Stock Materia Prima never needed any of this:** `deducir_materia_prima()` never updates an existing `InventarioRegistro` row — every consumption `INSERT`s a brand-new row carrying the resulting running total, tagged `"Consumo automatico: ..."`. A manual count's row is therefore immutable by construction from the moment it's written; there's no lot being drawn down in place, so no `cantidad`/`cantidad_original` split was ever needed there. This is a deliberate architecture difference, not an inconsistency to fix: materia prima is one running total per ingredient (no per-batch expiry), while congelado needs discrete, FIFO-orderable lots specifically for shelf-life/expiry tracking (see Stock Producto Congelado module) — that requirement is exactly what makes a congelado "count" row do double duty as both a historical record and a real consumable lot, which is the root of why only congelado needed a `cantidad_original` split.

### Manual counts now show up in "Movimientos recientes" (added 2026-09-10)
A manual stock count (`StockCongelado`/`InventarioRegistro` row with no automatic-notas prefix, see `es_conteo_manual()`) never gets its own `MovimientoStock` row — that's what lets the discrepancy check above mean something — but it's also exactly what `historial_movimientos_acumulado`'s re-anchoring reads, so a sudden jump/drop in calculado with NO visible cause in the activity feed was a recurring support question (Pan de Choco Cocinado jumping ~15u on a delivery day; a materia_prima ingredient's calculado going deeply negative). `conteos_manuales_movimientos(db, tipo_stock, producto_id, limit)` (`services/stock.py`) formats these as fake movement entries (`tipo_movimiento="conteo_fisico"`, `cantidad=None`, `saldo_despues=` the count) and gets spliced into the real `MovimientoStock`-based list in all three feeds: `GET /recetas/{id}/completo`, `GET /congelados/productos/{id}/detalle`, `GET /ingredientes/{id}/movimientos`.

**Same-day ordering matters and isn't just insertion order.** A conteo represents the LAST event of its day (counted at closing, after that day's mermas/entregas — confirmed with Felipe: "el inventario es lo que tiene que prevalecer al final de dia"), so within a tied date it must render above everything else that day. A `carga_inicial` is the opposite: it's a historical BASELINE that predates that date's real activity, even though the reconciliation script that inserts it runs (and gets an id) days or weeks after the fact. Both raw `id`-based tie-breaking (display order) and the `saldo_despues_por_movimiento()` replay (actual per-movement running-balance math) got this backwards for `carga_inicial` — found on Papa Deshidratada: a same-day `carga_inicial` displayed `saldo_despues≈0` and its consumo showed strongly negative, when it should've been the reverse ("había 125g" *then* consumed down to ~0). `orden_visual_dia(tipo_movimiento)` (`services/stock.py`) is the one shared rank used both by the SQL replay order in `saldo_despues_por_movimiento()` and by the Python-side merge-sort in all three feed endpoints: `conteo_fisico` sorts first (top), `carga_inicial` sorts last (bottom), everything else lands in between.

### Papa Deshidratada converted g → kg (2026-09-11)
Was the only ingredient tracked in grams while every other flour-family ingredient (Harina 000, Harina 00 Pizza, etc.) tracks in kg — small-magnitude gram values (125g of Papa Deshidratada per Pure batch) kept getting mistyped as if they were kg (typed "0.125" meaning 0.125kg, landed as 0.125g). `scripts/convertir_papa_deshidratada_a_kg.py` (dry-run by default, `--apply` to write) rescales every existing `InventarioRegistro`/`MovimientoStock` row for the ingredient (÷1000) and flips `unidad_compra`/`unidad_uso`/`cantidad_compra` in one transaction — `LineaReceta` needed no change, since a recipe line's own unit (still "125 g" on Receta 39/Pure) gets converted into whatever the ingredient's `unidad_uso` is at production time regardless. If another ingredient shows the same kg/g typo pattern, this script is the template (adjust `INGREDIENTE_ID`, everything else generalizes).

### Correcting a pedido's reception date (added 2026-09-03)
`PUT /api/pedidos/{id}` (generic field update) previously only wrote `Pedido.fecha_recepcion` — a real reception logged under the wrong date (e.g. entered today for a delivery that actually happened over a week ago) left the pedido saying the right date while its `InventarioRegistro`/`MovimientoStock` rows stayed dated with the original mistake, splitting the pedido from its own stock effect. Fixed: when `fecha_recepcion` changes on a pedido whose `estado == "recibido"`, the same request now also re-dates every `InventarioRegistro` row matched by `notas LIKE 'Pedido #{id} recibido%'` and every `MovimientoStock` row matched by `referencia_origen == 'pedido:{id}'` and `tipo_movimiento == 'recepcion'`. Quantities are untouched, only the date moves — this is a metadata correction, not a ledger balance adjustment, so it doesn't need the revert/reapply dance `entregas_b2b` uses.
`frontend/src/app/pedidos/[id]/page.tsx` didn't show `fecha_recepcion` anywhere before this either — added to the summary card with the standard safe inline-edit pattern (pencil on hover, explicit ✓/✕, no save-on-blur), gated behind `pedidos_proveedores`/`edit` like the page's other actions.
...
### Producción Analytics — extras and hours worked per day (added 2026-09-11/12)
`GET /api/produccion/analytics`'s "Por dia" breakdown only ever counted `TareaProduccion`-linked completions (`tipo="produccion"`, the fixed weekly plan), so a day where only off-calendar work happened — Pizza/Pan Lomo (see above: no `TareaProduccion` at all by design), or any weekday where the scheduled tasks just weren't done — either didn't appear in `por_dia` at all, or showed a misleading `0/N` with the real work invisible outside the disconnected "Producción no programada" list. Fixed in two passes:
- `por_dia` now includes `extra_count` (genuine production extras that day, same filter as `no_programada`: `tarea_id IS NULL` and `producto_congelado_id`/`receta_id` set) and `minutos_totales` (total `duracion_real` logged that day). A day is included if it has ANY registro at all, not just a scheduled production task.
- **Gotcha (found the next day, 2026-09-11):** the first version summed `minutos_totales` only from inside the `day_tareas` loop (i.e. only `tipo="produccion"` completions) plus extras — silently dropping any OTHER `TareaProduccion` `tipo` (e.g. `"admin"`, like "Guardar + Armar Pedido"). A day with 170 real minutes logged showed 50. Fixed by summing `duracion_real` across every `completada=True` `RegistroProduccion` for that date regardless of `tarea_id`/`tipo`, via a `registros_by_day` dict built once from the same range query already run for everything else on this endpoint.
`duracion_real` (an integer minutes field on `RegistroProduccion`, optional, no machine/human split) is the only production time-tracking that actually exists — the "tiempo de producción (maquinaria, humana)" sub-module and `POST /api/produccion/tiempos` described elsewhere in this file are aspirational/never built; don't assume they exist when working in this area.

**Extras bar rendering, went through three iterations (2026-09-11/12).** First: extras called out as separate blue text next to the bar ("+N extra"). Second: folded into the bar as inline text on top of it. Both read as disconnected from what they represented, or hid whether the extra work actually filled out the rest of the day. Current (final) version, `produccion/analytics/page.tsx`'s "Por dia": the bar itself is split into a green segment (`completadas/planificadas`, width capped at 100%) and, immediately after it, a blue segment sized by `extra_count/planificadas` (capped so the two together never exceed 100% width) — so a day like 4/5 planned + 1 extra visibly fills the ENTIRE bar instead of looking like an 80%-done day. The blue segment carries its own `+N` label in amber for legibility against blue; the `completadas/planificadas` text is always white with a `text-shadow` (previously it switched to gray when the green fill was small, e.g. Sábado's low-completion days, making it unreadable against the light track). Exact numbers (`X/Y planificadas + N extra`) are still available via the bar's `title` tooltip on hover.

### Historial discrepancy threshold is percentage-based, not a fixed 0.5 (changed 2026-09-10)
The `calculado`/`contado` pivot cells on `/congelados` and `/stock` used to flag red past a flat `DISCREPANCIA_TOLERANCIA = 0.5` absolute-unit gap — a 2-unit gap on a count of 500 (0.4%) got flagged exactly the same as a 2-unit gap on a count of 3 (67%). Replaced with `discrepanciaPct(calc, val)` (relative to whichever of `calc`/`val` is nonzero, `val` preferred as the physical ground truth): >10% is red (`text-red-600 bg-red-50`), 0–10% is orange (`text-orange-600 bg-orange-50`), differences under 0.005 absolute are floored to 0% to avoid flagging float/rounding noise invisible at `formatCantidad`'s 2-decimal display. Duplicated identically in both page files (same pattern as `valorCalculadoEnFecha`/`esConteoManual`) — a threshold change must be applied in both `congelados/page.tsx` and `stock/page.tsx`.

### `/produccion` day picker (added 2026-09-10)
A native `<input type="date">` sits next to the existing ‹ / Hoy / › day-nav arrows (kept, this is additive) so a specific date can be jumped to directly instead of paging one day at a time. Same `setFecha` + `window.history.replaceState` pattern as `changeDay`/`goToday` — no dedicated handler needed.
