...
**Reflecting a count with nothing moved since (added 2026-08-21).** The reset above only fired while iterating a movement dated AFTER the count — if nothing (no production/sale/merma) happened since, that line of the loop never runs, so calculado for "today" kept showing the stale pre-count trajectory forever, effectively ignoring the count. This was the common case, not the exception (confirmed live against 8 materia_prima ingredients). Fixed by emitting a trailing point the day after each still-unconsumed anchor, up to `fecha_hasta`/today. Relies on the frontend's own pivot lookup (`valorCalculadoEnFecha`, duplicated identically in `congelados/page.tsx` and `stock/page.tsx`: latest point with `fecha <= column`) to carry that single point forward through every later date column — no frontend change needed.
