...
- `por_dia` now includes `extra_count` (genuine production extras that day, same filter as `no_programada`: `tarea_id IS NULL` and `producto_congelado_id`/`receta_id` set) and `minutos_totales` (total `duracion_real` logged that day). A day is included if it has ANY registro at all, not just a scheduled production task. The frontend (`produccion/analytics/page.tsx`) shows extras INLINE in the bar itself (e.g. `"1/11 +3 extra"`, or the bar's own label on a no-plan day) — a separate "+N extra" pill next to the bar was tried first and rejected as reading disconnected from the day it described.
