...
**`deducir_congelado_fifo()` clamps on shortage, same as `deducir_materia_prima()`.** Requesting more than what's on hand records only what actually left the shelf (plus a "Stock insuficiente" note in `notas`), not the full theoretical demand — otherwise the ledger goes negative forever with nothing to reconcile against. Also flushes after each `StockCongelado` mutation for the same `autoflush=False` reason as materia_prima (see below).
...
- The "Evolucion de stock" line chart on `/congelados` was switched to plot `calculado` too (raw count only as fallback when an item has zero ledger activity) — most items have exactly ONE manual count in the whole date range, so Recharts could only ever draw a lone dot, never a line. **Not yet done for `/stock` (materia prima)'s equivalent chart** — only its pivot table got the calculated treatment.
